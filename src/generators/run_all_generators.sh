...
mkdir -p ./resources
mkdir -p ./resources/discrete/
mkdir -p ./resources/rational/
...
if [ -f "UniformGenerator.java" ]; then
    ...
    java UniformGenerator $NUM_SAMPLES ./resources/rational/java_uniform_0_1.txt
...
if [ -f "UniformGenerator.erl" ]; then
    escript UniformGenerator.erl $NUM_SAMPLES ./resources/rational/erlang_uniform_0_1.txt
else
...
if [ -f "UniformGenerator.py" ]; then
    python3 UniformGenerator.py $NUM_SAMPLES ./resources/rational/python_uniform_0_1.txt
else
...
if [ -f "SixSidedUniformGenerator.py" ]; then
    python3 SixSidedUniformGenerator.py $NUM_SAMPLES ./resources/discrete/python_dice_1_6.txt
else
...
if [ -f "FourSidedUniformGenerator.c" ]; then
    ...
    ./c_gen_4 $NUM_SAMPLES ./resources/discrete/c_discrete_1_4.txt
    rm c_gen_4
else
...
if [ -f "EightSidedUniformGenerator.c" ]; then
    ...
    ./c_gen_8 $NUM_SAMPLES ./resources/discrete/c_discrete_1_8.txt
    rm c_gen_8
else
...
if [ -f "UniformGenerator.rkt" ]; then
    racket UniformGenerator.rkt $NUM_SAMPLES ./resources/discrete/racket_integer_1_20.txt
else
...
if [ -f "UniformGenerator.pl" ]; then
    swipl UniformGenerator.pl $NUM_SAMPLES ./resources/rational/prolog_uniform_0_1.txt
else
